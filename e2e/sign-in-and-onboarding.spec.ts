...
test("redirects to onboarding when instance still needs onboarding", async ({
  page,
}) => {
...
test("shows validation messages when login is submitted with empty fields", async ({
  page,
}) => {
...
test("shows API error toast when login credentials are rejected", async ({
  page,
}) => {
  await page.goto("/login");
...
test("completes onboarding with Cloudflare provider and lands on dashboard", async ({
  page,
}) => {
  await page.request.post("/api/test/reset?seed=false");
  ...
  await expect(
    page.getByRole("heading", { name: "Configure Exposure Providers" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Exposure providers configured: Cloudflare"),
  ).toBeVisible();
  await page.getByRole("button", { name: "Get Started" }).click();

  await expect(
    page.getByText("Setup complete! Welcome to Labrador."),
  ).toBeVisible();
...
test("completes onboarding happy path without adoptable stacks — skips adopt step", async ({
  page,
}) => {
  await page.request.post("/api/test/reset?seed=false");
...
test("onboarding shows adopt step when unmanaged stacks exist", async ({
  page,
}) => {
  await page.request.post("/api/test/reset?seed=false");
  await page.request.post("/api/test/mock/docker", {
  ...
  await expect(
    page.getByRole("heading", { name: "Adopt Existing Stacks" }),
  ).toBeVisible();
