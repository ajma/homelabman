...
test("zero-state dashboard shows adoptable checklist when unmanaged stacks exist", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("button", { name: /adopt selected/i }),
  ).toBeVisible();
});

test("zero-state dashboard hides checklist when no adoptable stacks", async ({
  page,
}) => {
  await page.request.post("/api/test/mock/docker", {
    data: { containers: [] },
...
test("adopting from dashboard zero-state creates a project", async ({
  page,
}) => {
  await page.request.post("/api/test/mock/docker", {
    data: { containers: [unmanaged("myapp")] },
...
test("orphaned stack (has labrador label but no matching project) is adoptable", async ({
  page,
}) => {
  await page.request.post("/api/test/mock/docker", {
...
test("adopting stack with logo label restores logo on project", async ({
  page,
}) => {
  await page.request.post("/api/test/mock/docker", {
...
test("partial batch adoption returns correct adopted and failed arrays", async ({
  page,
}) => {
  await page.request.post("/api/test/mock/docker", {
    data: {
  ...
  await page.request.post("/api/projects/adopt", {
    data: { stackNames: ["badapp"] },
  });
