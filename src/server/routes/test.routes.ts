...
export async function testRoutes(app: FastifyInstance) {
  ...
  const mockCloudflare = (app as any)
    .cloudflareApiService as MockCloudflareApiService;
  ...
  app.post<{ Body: { containers?: any[]; images?: any[]; networks?: any[] } }>(
    "/mock/docker",
    async (request) => {
      if (request.body.containers !== undefined)
        mockDocker.containers = request.body.containers;
      if (request.body.images !== undefined)
        mockDocker.images = request.body.images;
      if (request.body.networks !== undefined)
        mockDocker.networks = request.body.networks;
      return { ok: true };
    },
  );
  ...
  app.post<{
    ...
    if (request.body.accounts !== undefined)
      mockCloudflare.accounts = request.body.accounts;
    if (request.body.tunnels !== undefined)
      mockCloudflare.tunnels = request.body.tunnels;
    if (request.body.nextTunnel !== undefined)
      mockCloudflare.nextTunnel = request.body.nextTunnel;
    return { ok: true };
  });
}
