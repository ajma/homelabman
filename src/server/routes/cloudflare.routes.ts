...
export async function cloudflareRoutes(app: FastifyInstance) {
  ...
  app.post<{ Body: { apiToken: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!apiToken)
        return reply.code(400).send({ error: "apiToken required" });
      try {
  ...
  app.post<{ Body: { apiToken: string; accountId: string } }>(
    "/tunnels",
    async (request, reply) => {
      ...
      if (!apiToken || !accountId)
        return reply
          .code(400)
          .send({ error: "apiToken and accountId required" });
      try {
  ...
  app.post<{
    Body: { apiToken: string; accountId: string; tunnelName: string };
  }>("/tunnels/create", async (request, reply) => {
    const { apiToken, accountId, tunnelName } = request.body;
    if (!apiToken || !accountId || !tunnelName) {
      return reply
        .code(400)
        .send({ error: "apiToken, accountId, and tunnelName required" });
    }
    try {
  ...
  app.post<{ Body: { apiToken: string; accountId: string; tunnelId: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!apiToken || !accountId || !tunnelId) {
        return reply
          .code(400)
          .send({ error: "apiToken, accountId, and tunnelId required" });
      }
      try {
