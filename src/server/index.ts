  ...
  try {
    ...
    if (dockerAvailable) {
      ...
      try {
        ...
        if (statusMap.size > 0) {
          app.log.info(
            `Reconciled ${statusMap.size} project status(es) from Docker`,
          );
        }
      } catch (reconcileErr) {
        app.log.warn(
          { err: reconcileErr },
          "Failed to reconcile project statuses",
        );
      }
    } else {
      app.log.warn(
        "Docker is not reachable; Docker features will be unavailable",
      );
      dockerService = null;
    }
  } catch (dockerErr) {
    app.log.warn(
      { err: dockerErr },
      "Failed to initialize Docker service; starting without Docker",
    );
    dockerService = null;
  }
