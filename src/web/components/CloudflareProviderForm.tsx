...
export function CloudflareProviderForm({
  ...
  useEffect(() => {
    if (!preselectedRef.current && detectedStack && accounts.length > 0) {
      ...
      onChangeRef.current({
        ...valueRef.current,
        adoptStackName: detectedStack.stackName,
  ...
  const handleConnect = async () => {
    ...
    try {
      const fetched = await api.post<CfAccount[]>("/cloudflare/accounts", {
  ...
  const handleAccountChange = async (accountId: string) => {
    ...
    try {
      const fetched = await api.post<CfTunnel[]>("/cloudflare/tunnels", {
        apiToken: value.apiToken,
        accountId,
      });
      setTunnels(fetched);
  ...
  const handleTunnelChange = (tunnelId: string) => {
    onChange({
      ...value,
      tunnelId,
      tunnelName: tunnelId === "__new__" ? value.tunnelName : "",
    });
  };
      ...
      <div className="space-y-1.5">
        ...
        <div className="space-y-1 text-xs text-muted-foreground">
          ...
          <ul className="list-disc list-inside space-y-0.5">
            ...
            <li>
              <span className="text-muted-foreground">Zone → Zone → Read</span>
            </li>
            <li>
              <span className="text-muted-foreground">Zone → DNS → Edit</span>
            </li>
          </ul>
        </div>
      </div>
        ...
        value.tunnelId === "__new__" && (
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) =>
                onChange({ ...value, tunnelName: e.target.value })
              }
            />
          </div>
        )}
          ...
          {detectedStack ? (
              ...
              {[
                {
                  ...
                  description:
                    "Register the running cloudflared stack as a Labrador infrastructure project.",
                ...
                },
                {
                  ...
                  description:
                    'Create a new "Cloudflare Tunnel" project and start a fresh cloudflared container.',
                  checked:
                    value.adoptStackName === null && value.deployContainer,
                  onSelect: () =>
                    onChange({
                      ...value,
                      adoptStackName: null,
            ...
            <div className="flex items-start gap-3">
              <input
                ...
                onChange={(e) =>
                  onChange({ ...value, deployContainer: e.target.checked })
                }
              />
              <label
                htmlFor="cf-deploy-container"
              ...
              >
                ...
                <p className="text-xs text-muted-foreground">
                  Creates a &quot;Cloudflare Tunnel&quot; project and starts the
                  cloudflared Docker container using the tunnel token. Required
                  for traffic to flow.
                </p>
              </label>
            </div>
