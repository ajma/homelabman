 ...
 */
export async function resolveCloudflareBeforeSave(
  cf: CloudflareProviderFormValue,
): Promise<{
  tunnelId: string;
  ...
  if (tunnelId === "__new__") {
    const result = await api.post<{ tunnelId: string; tunnelToken: string }>(
    ...
    const result = await api.post<{ tunnelToken: string }>(
      "/cloudflare/tunnels/token",
      {
        apiToken: cf.apiToken,
        accountId: cf.accountId,
        tunnelId,
...
export async function deployCloudflaredProject(
  tunnelToken: string,
): Promise<void> {
